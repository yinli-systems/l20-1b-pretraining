...
def worker_is_contaminated(text: str) -> bool:
    return is_contaminated(text, WORKER_CONTAMINATION_HASHES)


def worker_ignore_stop_signals() -> None:
...
def main() -> None:
    ...
    if contamination_hashes and args.workers > 1:
        process_pool = mp.get_context("fork").Pool(
            args.workers, initializer=worker_ignore_stop_signals
        )
    ...
    counters = {"seen": 0, "accepted": 0, "duplicate": 0, "contaminated": 0, "too_long": 0}
    ...
    try:
        ...
        for rows in batches(source_rows, args.batch_docs):
            ...
            for row in rows:
                ...
                if digest in pending_set or dedup.execute(
                    "SELECT 1 FROM documents WHERE hash = ?", (digest,)
                ).fetchone():
            ...
            if process_pool is None:
                contaminated = [is_contaminated(text, contamination_hashes) for text in texts]
            else:
                chunk_size = max(1, len(texts) // (args.workers * 4))
                contaminated = process_pool.map(worker_is_contaminated, texts, chunksize=chunk_size)
            clean_candidates = [
                candidate for candidate, contaminated_flag in zip(candidates, contaminated) if not contaminated_flag
            ]
            encodings = iter(tokenizer.encode_batch([text for text, _ in clean_candidates]))
            encoded_candidates = []
            for contaminated_flag in contaminated:
                encoded_candidates.append(None if contaminated_flag else next(encodings))
            ...
            for prepared_row in prepared:
                ...
                encoding = encoded_candidates[candidate_index]
                if encoding is None:
                    raise RuntimeError("missing tokenizer result for clean document")
                ids = encoding.ids
    ...
    if not train_writer.complete or not val_writer.complete:
        raise SystemExit(
