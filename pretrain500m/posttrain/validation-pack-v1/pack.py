"""Pack family-disjoint reserved documents with exact next-token loss masks."""
...
def pack_domain(item):
    ...
    token_path = Path(output) / f'development-{domain}.blocks.npy'
    mask_path = Path(output) / f'development-{domain}.loss-mask.npy'
    index_path = Path(output) / f'development-{domain}.documents.jsonl.gz'
    ...
    arrays = {}; cursor = 0; valid_targets = 0
    with gzip.open(index_path, 'xt', encoding='utf-8', compresslevel=1) as index:
        for record in records:
            ...
            if path not in arrays:
                ...
                arrays[path] = array
            doc = np.asarray(arrays[path][record['start']:record['end']], dtype=np.uint16)
            first = cursor
...
def run(args):
    ...
    if args.partition != 'development':
        raise ValueError('v1 packer is frozen to the development partition')
    ...
    for source in parent['sources']:
        sid = source['source_id']; reserved = source['outputs'][args.partition]
        for path_key, hash_key in (('path','sha256'),):
            if sha(reserved[path_key]) != reserved[hash_key]:
                raise ValueError(f'reserved stream changed: {sid}')
            bound[reserved[path_key]] = reserved[hash_key]
        ...
        with gzip.open(source['document_index'], 'rt', encoding='utf-8') as handle:
            for line in handle:
                ...
                if row['partition'] == args.partition:
                    row['stream_path'] = reserved['path']; docs.append(row)
        docs.sort(key=lambda row: row['start'])
        if len(docs) != reserved['documents'] or not docs or docs[0]['start'] != 0 or docs[-1]['end'] != reserved['encoded_tokens'] or any(a['end'] != b['start'] for a,b in zip(docs,docs[1:])):
    ...
    write(args.output/'launch.json', {'status':'PACKING_DOCUMENT_MASKED_DEVELOPMENT_NOT_ADMITTED',
    ...
    manifest = {
        'schema': 'p529m-packed-mixture-v3', 'purpose': 'document-masked-five-domain-development',
    ...
    manifest_path = args.output/'development-mixture.json'; write(manifest_path, manifest)
    final = {
        'status':'DOCUMENT_MASKED_FIVE_DOMAIN_DEVELOPMENT_COMPLETE_NOT_ADMITTED',
        ...
        'limitations':['Every document is packed independently; first document tokens and padding have no loss target.',
                       'Development data supports selection only and is not a sealed final test.',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--partition',default='development');parser.add_argument('--workers',type=int,default=4)
