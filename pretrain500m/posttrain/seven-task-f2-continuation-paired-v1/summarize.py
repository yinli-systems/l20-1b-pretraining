#!/usr/bin/env python3
"""Bind and summarize matched parent/continuation seven-task evaluations."""
...
def main() -> None:
    ...
    if plan.get("schema") != "p529m-seven-task-f2-continuation-paired-2gpu-plan-v1":
        raise ValueError("unexpected plan schema")
    if plan.get("status") != "FROZEN_BEFORE_THIS_COMPARISON":
    ...
    expected = {(role, seed) for role in ("parent", "continuation") for seed in (20260914, 20260915)}
    if {(item.get("role"), item.get("seed")) for item in plan["candidates"]} != expected:
        raise ValueError("candidate parent/continuation seed grid mismatch")
    for item in plan["candidates"]:
        if item["id"] != f"F2_{item['role']}_seed{item['seed']}":
            raise ValueError("candidate role/seed ID mismatch")
        if item["expected_step"] != (1024 if item["role"] == "parent" else 256):
            raise ValueError("candidate checkpoint step mismatch")
    ...
    by_role_seed = {(row["recipe"], row["seed"]): row for row in candidates}
    pairs = []
    for seed in (20260914, 20260915):
        parent = by_role_seed[("F2_parent", seed)]
        continuation = by_role_seed[("F2_continuation", seed)]
        task_deltas = {task: continuation["tasks"][task] - parent["tasks"][task] for task in TASKS}
        pairs.append({
            "seed": seed,
            "parent_checkpoint_sha256": parent["checkpoint_sha256"],
            "continuation_checkpoint_sha256": continuation["checkpoint_sha256"],
            "parent_aggregate_score": parent["aggregate_score"],
            "continuation_aggregate_score": continuation["aggregate_score"],
            "aggregate_delta_new_vs_parent": continuation["aggregate_score"] - parent["aggregate_score"],
            "task_delta_new_vs_parent": task_deltas,
            "strict_aggregate_gain": continuation["aggregate_score"] > parent["aggregate_score"],
            "no_task_decline_over_two_percentage_points": min(task_deltas.values()) >= -0.02,
        })
    mean_continuation = sum(row["aggregate_score"] for row in candidates if row["recipe"] == "F2_continuation") / 2
    progression_gate = {
    ...
    output = {
        "schema": "p529m-seven-task-f2-continuation-paired-2gpu-summary-v1",
        "status": "COMPLETE_MATCHED_TWO_GPU_BASE_AND_FOUR_CHECKPOINT_SEVEN_TASK_EVALUATION",
        ...
        "claim_boundary": "reused adaptive English seven-task progression and retention; not an independent sealed benchmark, instruction following, multilingual generation, safety or market superiority",
