...
def scan(inputs, bundle, output, scan_from_tranche=0):
    started = time.time()
    files, bindings, tranches = preflight(inputs)
    require(type(scan_from_tranche) is int and 0 <= scan_from_tranche < len(tranches),
            'invalid scan start tranche')
    scanned_files = [f for f in files if f['root_index'] >= scan_from_tranche]
    require(bool(scanned_files), 'no files selected for contamination scan')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scan-from-tranche', type=int, default=0)
    args = parser.parse_args()
    scan(args.input, args.bundle, args.output, args.scan_from_tranche)
