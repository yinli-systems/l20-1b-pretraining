"""Two-seed long confirmation runner. No automatic promotion path."""
import argparse
import contextlib
import hashlib
import json
...
def main():
    ...
    p.add_argument('--checkpoint-mode',choices=('full','model-only-final'),default='full')
    a=p.parse_args()
    ...
    if a.checkpoint_mode == 'model-only-final' and a.resume:
        p.error('model-only final screens are explicitly non-resumable')
    if not 1 <= a.pilot_steps <= 2048:
        p.error('this runner supports only 1..2048 confirmation steps per launch')
    ...
    required_free=(2*528748800*2+2*1024**3 if a.checkpoint_mode=='model-only-final'
    ...
    manifest={'status':'qualification_only',
      ...
      'checkpoint_semantics':('non-resumable model weights at successful final step only'
                              if a.checkpoint_mode=='model-only-final' else 'full resumable state')}
    ...
    # Fixed reduction layout qualified by recovery-v2 job 1588249.
    ...
    if a.validate_before_training:
        validation=evaluate_masked_domains(raw,val,rank,world,a.microbatch)
        if rank==0:
