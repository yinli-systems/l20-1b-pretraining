#!/usr/bin/env python3
"""Submit the completed N4 new-pool seven-task evaluations exactly once."""
...
SOURCE = ROOT / "source/new-pool-seven-task-eval-v2"
PLAN = SOURCE / "candidates.json"
RUNNER = SOURCE / "run.sbatch"
RECEIPT = ROOT / "receipts/new-pool-seven-task-eval-v2-submission.json"
JOB_PREFIX = "p529m-np7b-"
...
def write_receipt(status: str, jobs: list[dict], free: int, **extra) -> None:
    ...
    document = {
        "schema": "p529m-new-pool-seven-task-eval-submission-v2",
        "status": status,
...
def main() -> None:
    ...
    if plan.get("status") != "FROZEN_N4_BEFORE_EVALUATION":
        raise ValueError("evaluation plan is not frozen")
    candidates = plan.get("candidates", [])
    if len(candidates) != 1 or len({item["id"] for item in candidates}) != 1:
        raise ValueError("evaluation plan must contain one unique candidate")
