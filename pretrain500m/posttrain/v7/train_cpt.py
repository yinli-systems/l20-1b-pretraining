"""Prospective v7 fixed-static-graph qualification and throughput runner."""
import argparse
import contextlib
import hashlib
import json
...
def main():
    ...
    p.add_argument('--ddp-static-graph',action=argparse.BooleanOptionalAction,default=True)
    a=p.parse_args()
    ...
    manifest={'status':'qualification_only',
      ...
      'ddp_policy':{'find_unused_parameters':a.ddp_find_unused_parameters,'static_graph':a.ddp_static_graph,
    ...
    # V4 jobs observed no unused parameters on every rank. Static graph is the
    # prospective fast path that must also preserve the fixed-bucket invariant.
    model=torch.compile(raw);model=DDP(model,device_ids=[local],gradient_as_bucket_view=True,
                                     ...
                                     static_graph=a.ddp_static_graph,bucket_cap_mb=25)
    if state is not None:
