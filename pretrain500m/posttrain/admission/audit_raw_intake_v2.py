...
def preflight(inputs):
    """Bind complete inputs and reject repeated physical groups before scanning."""
    ...
    for ordinal, root in enumerate(roots):
        ...
        for receipt_path in receipts:
            sid = receipt_path.name.removesuffix('.receipt.json')
            receipt = bound_json(receipt_path, bindings)
            receipt_hash = bindings[receipt_path]
            require(receipt['id'] == sid and receipt['status'] == 'RAW_SEGMENT_READY',
                    'source is not complete: ' + sid)
            source = root / (sid + '.jsonl.gz')
            bindings[source] = sha(source)
            require(bindings[source] == receipt['output_sha256'], 'raw checksum mismatch: ' + sid)
            require(source.stat().st_size == receipt['output_bytes'], 'raw size mismatch: ' + sid)
            ...
            files.append(dict(path=source, receipt=receipt, groups=groups, source_id=sid,
                              tranche=ordinal, identity=identity))
...
def audit(inputs, tokenizer_path, expected_tokenizer_sha256, quotas_path, output):
    started = time.time()
    ...
    quotas = bound_json(quotas_path, bindings)
    require(quotas['sequence_length'] == 2048, 'unexpected packing length')
    required = {}
    for parents in quotas['screen'].values():
        for parent in parents.values():
            for leaf, value in parent['leaves'].items():
                required[leaf] = max(required.get(leaf, 0), (value['prediction_tokens'] + 1) // 2)
    for f in files:
        require(leaf_id(f['source_id']) in required, 'source absent from quota plan')
    ...
    with (output / 'row-index.jsonl').open('x') as index:
        for f in sorted(files, key=lambda x: (x['source_id'], x['tranche'])):
            ...
            file_counts.append(dict(source_id=sid, tranche=tranche, rows=nrows,
    ...
    report = dict(status='COMBINED_RAW_INTAKE_MEASURED_NOT_ADMITTED', training_admitted=False,
                  ...
                  tokenizer_sha256=expected_tokenizer_sha256, quota_plan_sha256=bindings[quotas_path],
                  ...
                  limits=['Raw upper bounds include one EOS per document, before filtering and held-out reserves.',
                          ...
                          'Two-epoch shortages cover one largest screen run, not cumulative screening and confirmation.'])
...
def main():
    ...
    report = audit(args.input, args.tokenizer, args.expected_tokenizer_sha256, args.quotas, args.output)
