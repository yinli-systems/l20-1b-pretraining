...
ROWS={};ROOTS=[];BINDINGS={};TOKENIZER=None;EOS=None;OUTPUT=None;RETENTION={}
...
def process_source(sid):
    ...
    with gzip.open(index_path,'xt',encoding='utf-8',compresslevel=1) as index:
        for tranche,root in enumerate(ROOTS):
            ...
            if str(path) not in BINDINGS:continue
...
def run(args):
    global ROWS,ROOTS,BINDINGS,OUTPUT,RETENTION
    ...
    raw_path=next(Path(p) for p in BINDINGS if p.endswith('/diverse-audit-v2-three-tranche/report.json'))
    ...
    total_bytes=sum(v['encoded_tokens']*2 for v in observed.values())
    ...
    write(args.output/'launch.json',dict(status='PACKING_SELECTED_CONTENT_NOT_ADMITTED',workers=args.workers,
        ...
        family_report_sha256=args.expected_family_sha256,training_admitted=False))
    sources=sorted({key[1] for key in observed})
    with mp.get_context('fork').Pool(args.workers,initializer=initialize,initargs=(str(args.tokenizer),)) as pool:
        results=list(pool.imap_unordered(process_source,sources))
