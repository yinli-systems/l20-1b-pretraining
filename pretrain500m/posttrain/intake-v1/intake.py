...
OUTPUT=ROOT/'data/diverse-intake-v1'
...
def acquire(segment):
    ...
    try:
        ...
        groups=sorted(range(parquet.num_row_groups),key=lambda i:hashlib.sha256((sid+str(i)+'p529m-intake-v1').encode()).hexdigest())
        with gzip.open(temp,'wt',encoding='utf-8',compresslevel=3) as writer:
            for group in groups[:4]:
                ...
                if sid.startswith('code_'):
                    ...
                    eligible=eligible[:max(0,5000-code_attempts)]
                ...
                if receipt.get('stop_reason') or code_attempts>=5000:break
...
def main():
    ...
    try:
        ...
        # Math first so useful content starts arriving promptly while code rehydrates.
        segments.sort(key=lambda x:(not x['id'].startswith(('finemath','infiwebmath')),x['id']))
