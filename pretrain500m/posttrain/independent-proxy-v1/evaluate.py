#!/usr/bin/env python3
"""Evaluate one native 529M checkpoint on a frozen Belebele proxy split."""
...
def bootstrap_accuracy(correct: list[bool], *, seed: int, resamples: int) -> list[float]:
    ...
    for _ in range(resamples):
        estimates.append(sum(correct[rng.randrange(size)] for _ in range(size)) / size)
...
def load_rows(data_path: Path, split_path: Path, role: str) -> list[dict]:
    ...
    for ordinal, (assignment, line) in enumerate(zip(split["rows"], raw_lines)):
        if assignment["ordinal"] != ordinal:
            raise ValueError("non-canonical split ordinal")
        if hashlib.sha256(line).hexdigest() != assignment["row_sha256"]:
            raise ValueError("row digest mismatch")
        row = json.loads(line)
        if assignment["role"] == role:
            selected.append({"ordinal": assignment["ordinal"], "row_sha256": assignment["row_sha256"], **row})
...
def encode_choices(tokenizer: Tokenizer, rows: list[dict], max_length: int) -> list[dict]:
    encoded = []
    for row in rows:
        context = f"Passage: {row['flores_passage']}\nQuestion: {row['question']}\nAnswer:"
        context_ids = tokenizer.encode(context).ids
        choices = []
        for index in range(1, 5):
            choice = " " + row[f"mc_answer{index}"]
            full_ids = tokenizer.encode(context + choice).ids
...
def main() -> None:
    ...
    report = {
        "schema": "p529m-independent-proxy-belebele-result-v1",
        "status": "INDEPENDENT_PROXY_DEVELOPMENT_EVALUATION_COMPLETE" if args.role == "development" else "INDEPENDENT_PROXY_CONFIRMATION_EVALUATION_COMPLETE",
