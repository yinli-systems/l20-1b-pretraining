...
does not create a training admission receipt. Up to sixteen CPU workers reuse the formal
...
Each worker owns separate source/tranche outputs and a private read-only language
model; the parent rechecks every bound input hash after all workers finish.
...
An explicit `--resume` reuses a shard only after its completed receipt, source
and tranche identity, row count, feature path, and feature SHA-256 all verify.
Partial shards are replaced, and a writer lock prevents concurrent resume
processes.

For an appended raw tranche, `--reuse-output` accepts a separate completed
quality prefix only when the prior quality report hash, raw incremental lineage,
exclusion-union lineage, raw bindings, shard receipts, and feature hashes all
match. New shard features are written to a fresh output directory.
