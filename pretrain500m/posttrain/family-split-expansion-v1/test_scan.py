...
class IntegrationTest(unittest.TestCase):
    def test_explicit_raw_file_mapping_avoids_root_index_alias(self):
        item={'source_id':'pdf_en','tranche':44}
        path=Path('/bound/physical/pdf-extra.jsonl.gz')
        self.assertEqual(scan.raw_path_for(item,{('pdf_en',44):path},[Path('/only-root')]),path)
        with self.assertRaisesRegex(ValueError,'raw file mapping missing'):
            scan.raw_path_for(item,{},[Path('/only-root')])
