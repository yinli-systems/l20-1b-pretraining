...
def raw_path_for(item, raw_files, inputs):
    key=(item['source_id'],item['tranche'])
    if key in raw_files:return raw_files[key]
    # Backward compatibility for the earlier one-file-per-root report schema.
    if 0<=item['tranche']<len(inputs):return inputs[item['tranche']]/(item['source_id']+'.jsonl.gz')
    raise ValueError(f'raw file mapping missing for {key}')
...
def run(args):
    ...
    raw_files={(item['source_id'],item['tranche']):Path(item['path']) for item in raw.get('files',[])}
    raw_work=[]
    for item in report['files']:
        path=raw_path_for(item,raw_files,inputs)
    ...
    final=dict(status='FAMILY_CLOSURE_AND_RESERVED_ASSIGNMENTS_COMPLETE_NOT_ADMITTED',checked_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
        ...
        raw_report_path=str(args.raw_report),raw_report_sha256=report['raw_report_sha256'],
