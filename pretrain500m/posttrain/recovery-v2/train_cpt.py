"""Frozen-FineWeb GPU recovery qualification only; no corpus/quality promotion."""
import argparse
import contextlib
import hashlib
import json
...
from recovery_trace import append_trace
...
def main():
    ...
    p.add_argument('--pilot-steps',type=int,required=True);a=p.parse_args()
    ...
    if min(a.microbatch,a.accumulation,a.target_tokens,a.validation_tokens,a.save_every,a.validate_every,a.mfu_window) <= 0:
    ...
    if (admission.get('schema') != 'p529m-frozen-data-engineering-v1' or
        admission.get('status') != 'CONTENT_VERIFIED_FOR_ENGINEERING_ONLY' or
        admission.get('frozen_manifest_sha256') != '29f75eca421f26653dd520e650d50e6a7802e9a499944b3b8382688fa3806e1a'):
        p.error('this runner only accepts the immutable original FineWeb pack for recovery testing')
    if digest(Path(admission['frozen_manifest'])) != admission['frozen_manifest_sha256']:
        p.error('immutable original data manifest mismatch')
    original=json.loads(Path(admission['frozen_manifest']).read_text())
    for current,original_key in ((a.mixture_manifest,'train_shards'),(a.validation_manifest,'validation_shards')):
        declared=json.loads(current.read_text())
        if declared['weights_percent'] != {'fineweb_edu':100} or len(declared['sources']) != 1:
            p.error('engineering qualification must use only the frozen FineWeb source')
        known={(x['path'],x['sha256'],x['tokens']//2049) for x in original[original_key]}
        for shard in declared['sources'][0]['shards']:
            if (shard['path'],shard['sha256'],shard['blocks']) not in known:
                p.error('engineering shard is not in the original frozen pack')
    if a.target_tokens > 134217728:
        p.error('recovery qualification is capped at 134217728 tokens per branch')
    ...
    required_free=2*checkpoint_reference.stat().st_size+2*1024**3
    if shutil.disk_usage(a.output_dir).free < required_free:
        p.error('insufficient disk headroom for two complete checkpoints plus 2 GiB margin')
    ...
    if a.target_tokens % tokens_per_step or a.validation_tokens % (world*a.microbatch*c.max_position_embeddings):
    ...
    code_hashes={'train_cpt.py':digest(Path(__file__)),
      ...
      **{name:digest(Path(__file__).parent/name) for name in ('mixture.py','resume.py','recovery_trace.py')}}
    manifest={'status':'qualification_only',
      ...
      'ddp_policy':{'find_unused_parameters':True,'static_graph':False,'gradient_as_bucket_view':True,'bucket_cap_mb':25},
    ...
    fingerprint_fields.update({key:manifest[key] for key in (
      ...
      'validation_manifest_sha256','admission_sha256','runtime','dense_bf16_tflops_per_gpu','mfu_minimum')})
    ...
    # Disable history-dependent bucket rebuilding so a fresh reducer after resume
    # uses the same reduction layout as the uninterrupted branch.
    model=torch.compile(raw);model=DDP(model,device_ids=[local],gradient_as_bucket_view=True,
                                     find_unused_parameters=True,static_graph=False,bucket_cap_mb=25)
    if state is not None:
    ...
    if a.validate_before_training:
        raw.eval();values=[]
        with torch.no_grad():
            for i in range(val.total_blocks//(world*a.microbatch)):
    ...
    while step<max_steps:
        run_steps+=1
        if step in (0,16):
            if rank==0:append_trace(a.output_dir,step,'before_update',{'model':raw.state_dict(),'optimizer':opt.state_dict()},model)
            dist.barrier()
        ...
        if step==16:
            append_trace(a.output_dir,step,'gradient_rank_'+str(rank),{n:p.grad for n,p in raw.named_parameters()},model)
            dist.barrier()
        norm=torch.nn.utils.clip_grad_norm_(raw.parameters(),1.0,error_if_nonfinite=True);opt.step();step+=1
        if step==17:
            if rank==0:append_trace(a.output_dir,step,'after_update',{'model':raw.state_dict(),'optimizer':opt.state_dict()},model)
            dist.barrier()
        ...
        if rank==0:
            record={'step':step,'prediction_tokens':step*tokens_per_step,'loss':loss_value,'lr':lr,
                    ...
                    'ddp_bucket_sizes':model._get_ddp_logging_data().get('bucket_sizes')}
        ...
        if step%a.save_every==0 or step==max_steps or stop_requested:
