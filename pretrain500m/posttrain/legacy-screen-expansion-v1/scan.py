...
def run(args):
    ...
    scan_from_tranche = getattr(args, 'scan_from_tranche', 0)
    ...
    if not 0 <= scan_from_tranche < len(tranches):
        raise ValueError('invalid scan start tranche')
    scanned_files = [f for f in files if f['root_index'] >= scan_from_tranche]
    if not scanned_files:
        raise ValueError('no files selected for legacy scan')
    ...
    atomic_json(args.output/'launch.json', dict(status='SCANNING', workers=args.workers,
                ...
                expected_rows=sum(f['receipt']['rows_written'] for f in scanned_files),
                scan_from_tranche=scan_from_tranche, training_admitted=False))
    ...
    if total != sum(f['receipt']['rows_written'] for f in scanned_files):
    ...
    report = dict(status='LEGACY_HASH_SCAN_COMPLETE_NOT_ADMITTED',
        ...
        scan_from_tranche=scan_from_tranche,
...
if __name__ == '__main__':
    ...
    p.add_argument('--scan-from-tranche', type=int, default=0)
