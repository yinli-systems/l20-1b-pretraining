#!/usr/bin/env python3
"""Bind and summarize frozen seven-task candidate evaluations."""
...
def main() -> None:
    ...
    if plan.get("schema") != "p529m-seven-task-confirmation-plan-v1":
        raise ValueError("unexpected plan schema")
    if plan.get("status") != "FROZEN_BEFORE_CANDIDATE_EVALUATION":
    ...
    if digest(args.base) != plan["base_aggregate_sha256"]:
        raise ValueError("immutable base aggregate hash mismatch")
    ...
    input_hashes = {
        ...
        "base_aggregate": digest(args.base),
    }
    ...
    output = {
        "schema": "p529m-seven-task-confirmation-summary-v1",
        "status": "PASS_FROZEN_FOUR_CANDIDATE_SEVEN_TASK_EVALUATION",
        ...
        "claim_boundary": "frozen English base-model multiple-choice accuracy and retention only; instruction following, multilingual generation, safety and market superiority remain unverified",
