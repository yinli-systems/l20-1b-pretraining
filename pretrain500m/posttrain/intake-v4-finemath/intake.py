...
OUTPUT=ROOT/'data/diverse-intake-v4-finemath'
SOURCE=Path(__file__).resolve().parent
http_ranges.CAP=128*1024**2
MAX_TEXT_BYTES=192*1024**2
MAX_ROWS=100000
MAX_CODE_ATTEMPTS=25000
# Aggregate of four active model-only atomic-writer gates plus 256 MiB for
# this bounded four-row-group intake.
MIN_FREE=17_049_915_392 + 256*1024**2
...
def main():
    ...
    try:
        ...
        if [x['id'] for x in segments] != ['finemath4']:
            raise ValueError('v4 intake is frozen to the bounded FineMath top-up')
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:receipts=list(pool.map(acquire,segments))
