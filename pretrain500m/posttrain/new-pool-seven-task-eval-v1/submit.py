#!/usr/bin/env python3
"""Submit the first four new-pool seven-task evaluations exactly once."""
...
SOURCE = ROOT / "source/new-pool-seven-task-eval-v1"
PLAN = SOURCE / "candidates.json"
RUNNER = SOURCE / "run.sbatch"
RECEIPT = ROOT / "receipts/new-pool-seven-task-eval-v1-submission.json"
JOB_PREFIX = "p529m-np7-"
...
def write_receipt(status: str, jobs: list[dict], free: int, **extra) -> None:
    ...
    document = {
        "schema": "p529m-new-pool-seven-task-eval-submission-v1",
        "status": status,
        ...
        "claim_boundary": "submitted adaptive seven-task screens; scheduler allocation, completion, capability gain, and promotion remain unverified",
        **extra,
    }
...
def main() -> None:
    ...
    if plan.get("status") != "FROZEN_FIRST_FOUR_COMPLETED_ARMS_BEFORE_EVALUATION":
        raise ValueError("evaluation plan is not frozen")
    candidates = plan.get("candidates", [])
    if len(candidates) != 4 or len({item["id"] for item in candidates}) != 4:
        raise ValueError("evaluation plan must contain four unique candidates")
