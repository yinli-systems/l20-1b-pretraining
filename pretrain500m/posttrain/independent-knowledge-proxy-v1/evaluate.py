#!/usr/bin/env python3
"""Score one native 529M checkpoint on the frozen closed-book SciQ proxy."""
...
def bootstrap_accuracy(correct: list[bool], *, seed: int, resamples: int) -> list[float]:
    ...
    size = len(correct)
    estimates = [sum(correct[rng.randrange(size)] for _ in range(size)) / size for _ in range(resamples)]
...
def load_rows(data_path: Path, split_path: Path, role: str) -> list[dict]:
    ...
    for ordinal, (assignment, line) in enumerate(zip(split["rows"], raw_lines)):
        ...
        if assignment.get("role") != role:
            continue
        row = json.loads(line)
        source_options = [row["correct_answer"], row["distractor1"], row["distractor2"], row["distractor3"]]
        ...
        ordered_options = [by_hash[value] for value in assignment["choice_text_sha256"]]
        gold = ordered_options.index(row["correct_answer"])
...
def encode_choices(tokenizer: Tokenizer, rows: list[dict], max_length: int) -> list[dict]:
    encoded = []
    for row in rows:
        ...
        for answer in row["choices"]:
            completion = " " + answer
            full_ids = tokenizer.encode(context + completion).ids
            ...
            if not target_ids or len(full_ids) > max_length:
                raise ValueError(f"invalid sequence length at row {row['ordinal']}")
            choices.append({"ids": full_ids, "target_start": len(context_ids), "target_tokens": len(target_ids)})
        encoded.append({
            "ordinal": row["ordinal"],
...
def score(model: LlamaForCausalLM, encoded: list[dict], *, batch_size: int, pad_id: int) -> list[dict]:
    flat = []
...
def main() -> None:
    ...
    for path, expected in [(args.data, plan["data"]["sha256"]), (args.splits, plan["split_manifest_sha256"])]:
        if digest(path) != expected:
            raise ValueError(f"input digest mismatch: {path}")
    checkpoint = Path(candidate["checkpoint"])
    if digest(checkpoint) != candidate["checkpoint_sha256"]:
    ...
    if state.get("step") != candidate["expected_step"]:
        raise ValueError("checkpoint step mismatch")
    native_state = state["model"]
    del state
    ...
    loaded = model.load_state_dict(native_state, strict=True)
    ...
    results = score(model, encoded, batch_size=plan["execution"]["batch_size_per_gpu"],
                    pad_id=native_config.eos_token_id)
    del model
    ...
    for metric in ["acc_norm", "acc"]:
        ...
        metrics[metric] = {
            ...
            "correct": sum(correct),
            "samples": len(correct),
            "bootstrap_ci95": bootstrap_accuracy(correct, seed=plan["bootstrap"]["seed"],
                                                   resamples=plan["bootstrap"]["resamples"]),
        }
    report = {
        "schema": "p529m-independent-knowledge-proxy-sciq-result-v1",
        "status": "INDEPENDENT_KNOWLEDGE_PROXY_DEVELOPMENT_COMPLETE" if args.role == "development" else "INDEPENDENT_KNOWLEDGE_PROXY_CONFIRMATION_COMPLETE",
