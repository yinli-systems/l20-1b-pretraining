...
class BoundedHTTPFile(io.RawIOBase):
    def __init__(self,url):
        ...
        retry=Retry(total=4,connect=3,read=2,status=3,backoff_factor=0.25,
                    status_forcelist=(429,500,502,503,504),allowed_methods=frozenset({'GET'}))
        self.session.mount('https://',HTTPAdapter(max_retries=retry,pool_connections=2,pool_maxsize=2))
        self.total=None
        self.fetch(0,1)
    def fetch(self,start,n):
        ...
        if n==0: return b''
        with self.session.get(self.url, headers={'Range':f'bytes={start}-{start+n-1}',
                              'Accept-Encoding':'identity'},stream=True,timeout=(5,30)) as res:
