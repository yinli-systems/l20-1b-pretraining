#!/usr/bin/env python3
"""Submit the matched F2 parent and two fresh-N1 arms on RTX 4090 exactly once."""
...
SOURCE = ROOT / "source/f2-fresh-n1-seven-task-4090-v1"
PLAN = SOURCE / "candidates.json"
RUNNER = SOURCE / "run.sbatch"
RECEIPT = ROOT / "receipts/f2-fresh-n1-seven-task-4090-v1-submission.json"
JOB_PREFIX = "p529m-f2n1-4k7-"
...
def write_receipt(status: str, jobs: list[dict], free: int, **extra) -> None:
    capacity = subprocess.run(
        ["sinfo", "-p", "gpu_4090"], text=True, capture_output=True
    ).stdout
    document = {
        "schema": "p529m-f2-fresh-n1-4090-seven-task-submission-v1",
        "status": status,
        ...
        "claim_boundary": "submitted three-way RTX 4090 matched seven-task screens; scheduler allocation, completion, capability gain, and promotion remain unverified",
        **extra,
    }
...
def main() -> None:
    ...
    if plan.get("status") != "FROZEN_THREE_WAY_4090_MATCHED_EVALUATION":
