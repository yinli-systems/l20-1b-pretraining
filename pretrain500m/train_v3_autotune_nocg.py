...
def main():
    ...
    p.add_argument('--ddp-gradient-reduction',choices=('fp32','bf16'),default='fp32')
    ...
    model=torch.compile(raw,mode='max-autotune-no-cudagraphs');model=DDP(model,device_ids=[local],gradient_as_bucket_view=True,bucket_cap_mb=a.ddp_bucket_cap_mb)
    if a.ddp_gradient_reduction=='bf16':
    ...
    code_hashes={'train.py':digest(Path(__file__)),'model.py':digest(Path(__file__).with_name('model.py')),'data.py':digest(Path(__file__).with_name('data.py'))}
    manifest={'status':'pilot' if a.pilot_steps else 'formal','from_scratch':True,'initial_checkpoint':None,
      ...
      'compile_mode':'max-autotune-no-cudagraphs','deterministic_algorithms':a.deterministic,'ddp_gradient_reduction':a.ddp_gradient_reduction,
      'ddp_bucket_cap_mb':a.ddp_bucket_cap_mb,
    ...
    fingerprint_fields={key:manifest[key] for key in (
      ...
      'data_manifest_sha256','protocol_sha256','compile_mode','deterministic_algorithms','ddp_gradient_reduction','ddp_bucket_cap_mb')}
