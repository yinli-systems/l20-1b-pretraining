...
def main():
    ...
    model=torch.compile(raw,mode='max-autotune');model=DDP(model,device_ids=[local],gradient_as_bucket_view=True,bucket_cap_mb=a.ddp_bucket_cap_mb)
    ...
    manifest={'status':'pilot' if a.pilot_steps else 'formal','from_scratch':True,'initial_checkpoint':None,
      ...
      'compile_mode':'max-autotune','deterministic_algorithms':a.deterministic,'ddp_gradient_reduction':a.ddp_gradient_reduction,
