...
def main():
    ...
    model=torch.compile(raw);model=DDP(model,device_ids=[local],gradient_as_bucket_view=True,bucket_cap_mb=200,static_graph=True)
    ...
    manifest={'status':'pilot' if a.pilot_steps else 'formal','from_scratch':True,'initial_checkpoint':None,
      ...
      'deterministic_algorithms':a.deterministic,'ddp_gradient_reduction':'bfloat16_compress_hook','ddp_bucket_cap_mb':200,'ddp_static_graph':True,
    ...
    fingerprint_fields={key:manifest[key] for key in (
      ...
      'data_manifest_sha256','protocol_sha256','deterministic_algorithms','ddp_gradient_reduction','ddp_bucket_cap_mb','ddp_static_graph')}
