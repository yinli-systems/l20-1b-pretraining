...
def main():
    ...
    # Scale-only probe: initialize directly on the selected GPU to avoid slow host RNG on legacy nodes.
    with torch.device('cuda'):
        raw=LM(c)
    params=sum(x.numel() for x in raw.parameters());assert params==c.parameter_count()
    ...
    code_hashes={name:digest(Path(__file__).with_name(name)) for name in ('train.py','model.py','data.py')}
