#!/usr/bin/env python3
"""Fail-closed protocol-v3 formal-training preflight and Slurm dispatch."""
...
PROTOCOL_SHA256 = "38a8adbbba3ca19d2cd59718412fb6671925b6b123350d25a8755eed8661b893"
...
def main() -> None:
    ...
    protocol = ROOT / "source" / "protocol-v3.json"
    ...
    for receipt_path in sorted((ROOT / "pilots").glob("scale-v*-*-nodes-*/scale-receipt.json")):
        ...
        record["admitted"] = bool(
            ...
            and record["protocol_sha256"] in {PILOT_PROTOCOL_SHA256, PROTOCOL_SHA256}
        )
    ...
    training_job, training_launch = submit(
        [
            ...
            str(ROOT / "source" / "train-v2" / "train_ngpu_v3.sbatch"),
        ]
    )
    export_job, export_launch = submit(
        [
            f"--dependency=afterok:{training_job}",
            str(ROOT / "source" / "train-v2" / "export_hf_v3.sbatch"),
        ]
    )
    evaluation_job, evaluation_launch = submit(
        [
            f"--dependency=afterok:{export_job}",
            str(ROOT / "source" / "train-v2" / "evaluate_4gpu_v3.sbatch"),
        ]
    )
    receipt = {
    ...
    output = ROOT / "formal" / "formal-dispatch-v3.json"
