...
### Eight-way expert-parallel qualification

`cvcr_moe/train_ep.py` is a bounded 8xRTX5090 screen for the user's selected
plain Top-2 7B direction. Each rank retains exactly two of the 16 globally
initialized experts; autograd-aware variable-split all-to-all sends Top-2
assignments to their owner and returns outputs to the source-token order.
Replicated attention/router/embedding gradients use DDP while disjoint expert
gradients are excluded from DDP and globally mean-scaled once. The screen keeps
131,072 prediction tokens per optimizer step and rejects results below 50%
causal-useful MFU or 2 GiB per-GPU memory headroom.

This first screen deliberately uses pure BF16 parameters, gradients, and Adam
moments to establish the performance frontier. A pass is only a candidate for
matched all-tensor update parity, a higher-precision master-weight policy,
rank-sharded checkpoint/restore, and a sustained run. It cannot launch the
formal 150B-token training plan by itself.
