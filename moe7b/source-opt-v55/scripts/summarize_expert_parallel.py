"""Apply fail-closed MFU and memory gates to an expert-parallel screen."""
...
def main() -> None:
    ...
    experts_per_rank = int(manifest["experts_per_rank"])
    gates = {
        "complete": status.get("status") == "COMPLETED",
        "all_finite": all_finite,
        "verified_device_count": len(manifest["runtime"]["devices"]) == world_size,
        "supported_expert_parallel_degree": world_size in (4, 8),
        "expert_parallel_degree_matches_world": (
            int(manifest["expert_parallel_degree"]) == world_size
        ),
        "experts_partition_evenly": experts_per_rank * world_size == 16,
