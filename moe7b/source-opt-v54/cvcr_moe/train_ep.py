...
def main() -> None:
    ...
    if world_size != 8:
        raise ValueError("the frozen screen requires exactly eight expert-parallel ranks")
    if arguments.microbatch * arguments.accumulation != 8:
        raise ValueError("the screen must preserve eight sequences per rank per step")
    ...
    if any(row is None or row["device"] != arguments.required_device for row in devices):
        raise RuntimeError("the screen did not receive eight exact requested GPUs")
